...
/**
 * 版本感知适配层 —— 让同一份字节码在多个 MC 子版本上运行（Fabric 侧实现）。
 *
 ...
 *   <li>旧权限（1.21.2~1.21.10）：{@code ServerCommandSource.hasPermissionLevel(int)}
 *       （intermediary {@code method_9259}）</li>
 *   <li>新权限（1.21.11）：{@code getPermissions().hasPermission(new Permission.Level(...))}
 ...
 *
 * <p>实测锚点：tryBreakBlock 的 intermediary 在 1.21.5 / 1.21.10 / 1.21.11 均为
 * {@code method_14266}（Mixin target 跨版本稳定）；fabric-api 的
 * HudRenderCallback / ServerTickEvents / PayloadTypeRegistry / CommandRegistrationCallback
 * 在 1.21.2~1.21.11 均存在。
 */
public final class VersionCompat {
    ...
    public static boolean hasOpLevel2(ServerCommandSource source) {
        try {
            // 旧权限（1.21.2~1.21.10）：hasPermissionLevel(int)
            Method m = source.getClass().getMethod(HAS_PERMISSION_LEVEL, int.class);
            return (Boolean) m.invoke(source, 2);
        } catch (NoSuchMethodException e) {
            // 新权限（1.21.11+）：getPermissions().hasPermission(new Permission.Level(PermissionLevel.GAMEMASTERS))
